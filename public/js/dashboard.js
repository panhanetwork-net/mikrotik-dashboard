...
async function fetchPppoe() {
  ...
  if (data.error) throw new Error(data.error);
  allPppoe = Array.isArray(data) ? data : [];
  document.getElementById('pppoe-total').textContent = allPppoe.length;
  document.getElementById('pppoe-online').textContent = allPppoe.length;
  document.getElementById('pppoe-total-badge').textContent = `${allPppoe.length} sesi`;
  const badge = document.getElementById('pppoe-badge');
  badge.textContent = allPppoe.length;
  badge.style.display = allPppoe.length ? 'inline' : 'none';
  renderPppoeTable(allPppoe);
}
...
function filterPppoe() {
  ...
  renderPppoeTable(q ? allPppoe.filter(s => JSON.stringify(s).toLowerCase().includes(q)) : allPppoe);
}
